...
> 本文依据当前工作区源码整理，重点描述“代码实际上如何运行”，而不是仅复述项目提案。分析基准：2026-08-02。
...
| `scripts/verify_*.py` | 布局、传感器、路径、避碰、充电、CBS 等专项验证 |
...
8. 周期性执行空闲机器人迁移、主动轨迹冲突扫描、停滞重规划与安全迁移。
...
5. 安全迁移：达到 `STALL_RELOCATION_TIMEOUT=5s` 且无法恢复的相邻停滞机器人按组迁移到经障碍、同伴间距和落点检查的安全位置。
...
- `RobotState.WAITING` 是预留状态，当前主链路并不主动进入它；不要把它当成已完整实现的业务阶段。
- 运行时 RHCR 默认关闭且候选不会事务式下发，不应在论文或报告中描述为默认在线控制器。
- `factory_supervisor.py` 中初始化位置字典与 `config.py` 的 `PARKING_SPOTS` 并不完全一致；运行后位姿会由 Webots 场景值刷新，但启动前规划、文档和验证应以 `.wbt` 实体与 `PARKING_SPOTS` 的一致性为准。
- Supervisor 配置的最大线速度为 0.26 m/s，而机器人本地控制器实际限制为 0.22 m/s；时空预计和实验解释应采用执行端 0.22 m/s，或统一两处参数。
- 源码部分历史注释存在乱码，但标识符、控制逻辑和 JSON 协议本身不受影响；维护时建议统一保存为 UTF-8。
- `tests` 在当前工作树中处于已删除状态；现存 `scripts/verify_*.py` 是主要专项验证入口，但它们不等于完整单元测试套件。
