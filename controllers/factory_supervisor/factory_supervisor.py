...
class FactorySupervisor:
    ...
    def _refresh_joint_grid_candidate(self):
        ...
        if candidate is not None:
            ...
            # Preserve the last safe route and retry on the next rolling tick.
            # Freezing the whole active set turns a planning miss into a
            # guaranteed fleet-wide stall, so keep the old joint plan moving.
    ...
    def _joint_predictive_speed_shield(self, active_robots) -> bool:
        """Pre-empt soft-plan proximity with coordinated speed shaping.

        This is the runtime counterpart to the strict 3-cell joint planner.
        When a dense start forces the soft 2-cell fallback, predicted
        trajectories are re-checked every watchdog tick and one component at a
        time is slowed *before* any robot gets close enough to brake or lock.
        It never issues reverse/hold commands, so it cannot create the
        stationary-blocking behaviour that previously froze the fleet.
        """
        now = self.sim_time
        ...
        # Actual pose fallback.  Pure-pursuit can cut a corner relative to the
        # planned centreline, so trajectory prediction alone is not enough.
        # This layer only slows one deterministic robot; it never reverses or
        # holds and therefore cannot produce a stationary blocking pair.
        actual_conflicts = []
        ...
        if actual_conflicts:
            ...
            for component in actual_components:
                ...
                yielder = component[-1]
                for rid in component:
                    scale = 0.55 if rid == yielder else 0.85
